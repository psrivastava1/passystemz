...
//$url="http://bulksms.niktechsoftware.com/vendorsms/pushsms.aspx?user=PASS&password=pass12&msisdn=".$number."&sid=PASYSM&msg=".urlencode($msg)."&fl=0&gwid=2";
	$url="http://zapsms.co.in/vendorsms/pushsms.aspx?user=PASS&password=pass12&msisdn=".$number."&sid=PASYSM&msg=".urlencode($msg)."&fl=0&gwid=2";
	$ch = curl_init();
...
$url = "http://zapsms.co.in/vendorsms/CheckBalance.aspx?user=PASS&password=pass12";
$ch = curl_init();
curl_setopt($ch,CURLOPT_URL,$url);
curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
$output=curl_exec($ch);
