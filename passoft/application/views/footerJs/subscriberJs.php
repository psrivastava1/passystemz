<!-- start: MAIN JAVASCRIPTS -->
 ...
 //alert(panVal);
 $.post("<?php echo site_url('subscriberController/match_pan1') ?>", {panVal : panVal}, function(data){
...
if(regpan.test(panVal)){
 ...
 /////subscriber Code
 $("#joinerpass").keyup(function(){
              var cat = $("#joinerid").val();
              var pass = $("#joinerpass").val();
             // alert(cat);
             if((cat.length>0) && (pass.length)>0)
             {
              $.post("<?php echo site_url("subscriberController/checkjoinerid") ?>",{cat : cat , pass : pass}, function(data){
                $('#message1').show();
                $("#aarju").html(data);
                });   
             }
