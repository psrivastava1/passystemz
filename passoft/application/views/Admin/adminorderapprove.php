...
<div class="row">
                  ...
                  <tr class="text-uppercase">
                    <td><?php echo $i;?></td>
                      ...
                      <?php 
                       ...
                       <?php  $this->db->where('id',$custdetail->district);
                             $branch=$this->db->get('branch'); 
                             if($branch->num_rows()>0){

                             
                             ?>
                        <td><?php echo $branch->row()->b_name; } else { echo "N/A"; }  ?></td>

                        <?php  $this->db->where('id',$row->sub_branchid);
                             $sub_branch=$this->db->get('sub_branch'); 
                             if($sub_branch->num_rows()>0) { ?>
                        <td><?php echo  $sub_branch->row()->bname; } else { echo "N/A"; } ?></td> 
  ...
  <option class="text-uppercase" style="color:#01a9ac" value="<?php echo $row1->id;?>" <?php if($boy1->del_boy_id == $row1->id): echo 'selected="selected"'; endif; ?>><?php echo  $row1->name." [ ". $row1->username. " ] ";?></option>     
    ...
    <?php }?></select><?php }?></td>
            ...
            <td><?php if($row->order_status == 0) { ?><button type="submit" class="btn btn-danger" >Not Deliver </button><?php } else { ?><button type="submit" class="btn btn-warning" >Delivered </button><?php }?></td> 
         ...
         $('#payment<?php echo $i;?>').hide();
                ...
                endforeach;
                   ?>
                                                    ...
                                                    <center><div><a href="<?php echo base_url();?>adminController/index" class="btn btn-success"><span style="font-size:35px;">Done</span></a></div></center>
                                                </div>
                                                <?php } 
                                                elseif($uri==5){
                                                ?>
                 ...
                 $dt= $this->db->get("order_serial")->result();
                  ...
                  foreach($dt as $row):
                            $this->db->select_sum('quantity');
                             $this->db->select_sum('subtotal');
                             $this->db->where('order_no',$row->order_no);
                             $dt_2= $this->db->get("order_detail");
                             if($dt_2->num_rows()>0)
                             {
                                $dt1 = $dt_2->row();
                                $this->db->where('id',$row->cust_id);
                                $custdetail_2 =$this->db->get('customers');
                                if($custdetail_2->num_rows()>0)
                                {
                                    $custdetail = $custdetail_2->row();
                                    $this->db->where('id',$custdetail->district);
                                    $branch_2=$this->db->get('branch');
                                    if($branch_2->num_rows()>0)
                                    {
                                        $branch = $branch_2->row();
                                        $this->db->where('id',$row->sub_branchid);
                                        $sub_branch_2 = $this->db->get('sub_branch');
                                        if($sub_branch_2->num_rows()>0)
                                        {
                                            $sub_branch = $sub_branch_2->row();
                  ...
                  <tr class="text-uppercase">
                    <td><?php echo $i;?></td>
                      ...
                      <?php 
                       ...
                       if($branch->num_rows()>0){
                       ?>
                        <td><?php echo $branch->row()->b_name;?></td>
                      
                     <?php } else { ?> <td> <?php echo "N/A";?> </td> <?php }?>
                        <td><?php echo  $sub_branch->bname;?></td> 
                      <td><?php echo $dt1->quantity;?></td>
                      <td><?php echo $dt1->subtotal;?></td>
                      <td><?php echo $row->order_date;?></td>
