      ...
      <table id="customer_data" class="table table-bordered table-striped table-responsive">
        <thead>
          <tr>
            ...
            <th style="width:8px;">Prod.Name</th>
            <th style="width:8px;">Prod.Code</th>
            <th style="width:8px;">Size</th>
            <th style="width:8px;">Image</th>
            <th style="width:8px;">Rec.Qty</th>
            <th style="width:8px;">Total Qty</th>
          </tr>
        </thead>
        <tbody>
          <?php   
                  ...
                  $sproduct=$this->db->get('branch_wallet')->result();
                  $i=1;
                    foreach($sproduct as $value):

                   $this->db->where('id',$value->p_code);
                   $pronm=$this->db->get('stock_products')->row(); 
                  ?>
          <tr class="text-uppercase" style="width:8px;">
            <td style="width:8px;color:black;"><?php echo $i;?></td>
            <td style="width:8px;color:black;padding:.2rem;white-space:normal;"><?php echo  $pronm->name;?>
            </td>
            <td style="width:8px;color:black;"><?php echo $value->p_code?> </td>
               <td style="width:8px;color:black;"><?php echo $pronm->size?> </td>
                  <td style="width:8px;color:black;"><img src="<?php echo $this->config->item('asset_url'). '/productimg/' . $pronm->file1; ?>"
        ...
        $this->db->where('DATE(date)',$dt);
        $this->db->where('reciver_usernm',$branch->username);
        ...
        if($product_trans->num_rows()>0){
          $product_trans1=$product_trans->row();  ?>
            ...
            <td style="width:8px;color:black;"><?php echo $product_trans1->quantity;?></td>
