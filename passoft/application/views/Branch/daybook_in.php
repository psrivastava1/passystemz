<?php if($this->session->userdata("login_type")==1) { ?>
      	            <div>
      	                <strong><center> <h2 style='color: green'> IN Daybook Record From Branch & Shop To Admin </h2></center></strong>
                    </div>
<?php } elseif($this->session->userdata("login_type")==2) { ?>
      	            <div>
      	                <strong><center> <h2 style='color: green'> IN Daybook Record From Admin & Shop To Branch </h2></center></strong>
                    </div>
<?php } elseif($this->session->userdata("login_type")==4) { ?>
      	            <div>
      	                <strong><center> <h2 style='color: green'> IN Daybook Record From Admin & Branch To Shop </h2></center></strong>
                    </div>
<?php }?>
				    ...
				    <div class="row">
						...
						<script>
							$("#sub_1").click(function(){
								...
								// alert(uname);
								<?php if($this->session->userdata("login_type")==1) { ?>
								$.post("<?= site_url();?>adminController/indaybook_record",{uname:uname},function(data){
									$("#show_1").html(data);
								});
							<?php } elseif($this->session->userdata("login_type")==2) { ?>
								$.post("<?= site_url();?>branchController/indaybook_record",{uname:uname},function(data){
									$("#show_1").html(data);
								});
							<?php } elseif($this->session->userdata("login_type")==4) { ?>
								$.post("<?= site_url();?>shopController/indaybook_record",{uname:uname},function(data){
									$("#show_1").html(data);
								});
							<?php } ?>
							});
