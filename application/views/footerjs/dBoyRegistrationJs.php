...
if(regpan.test(panVal)){
	...
	// $('#messagepan').html('invalid pan card number').css('color', 'red');
	 $('#messagepan').html('<ul style="list-style-type:disc;"><li>first 5 digit should be alphabetic</li><li>then 4 digit should be numeric</li><li>last 1 digit should be alpabetic</li></ul>').css('color', 'red');
...
$('#aadhar').change(function(){
 ...
 //alert(adhar);
$.post("<?php echo site_url('auth/match_adhar') ?>", {adhar : adhar
        }, function(data){
              $('#aadhardeli').html(data);
                document.getElementById("subbutton").disabled = false;
       });
});
